...
function createInstanceId(cardId) {
...
function findPlayedCard(game, target) {
    const player = game.players[target.playerId];
    return [...player.monsterZone, ...player.spellZone].find((card) => card.instanceId === target.instanceId);
}
function findMonster(game, target) {
    return game.players[target.playerId].monsterZone.find((card) => card.instanceId === target.instanceId);
}
...
export function getTargetRequirement(card) {
...
export function canTargetMonster(game, playerId, target, requirement) {
...
function applySpellEffect(game, playerId, spell) {
    ...
    if (spell.effectType === "buff" && targetMonster) {
        let amount = 300;
        if (spell.id === "motivationsspritze" || spell.id === "pomodoro-technik")
            amount = 600;
    ...
    if (spell.id.startsWith("gefuehl-")) {
        ...
        if (targetMonster) {
            let buffAmount = 100;
            if (gefuehlCount === 2)
                buffAmount = spell.deck === "finger" ? 300 : 500;
            if (gefuehlCount >= 3)
                buffAmount = spell.deck === "finger" ? 500 : 1000;
...
export function playCardFromHand(game, playerId, cardId, target) {
    ...
    if (targetRequirement) {
        if (!target)
            return game;
        const isValidTarget = canTargetMonster(nextGame, playerId, target, targetRequirement);
        if (!isValidTarget)
            return game;
    }
...
export function revealStagedCards(game) {
    const nextGame = cloneGame(game);
    nextGame.phase = "reveal";
    const allStagedSpells = [];
...
export function getWinner(game) {
    if (game.phase !== "gameEnd")
        return undefined;
    const player1Strength = game.players.player1.monsterZone.reduce((sum, monster) => sum + monster.currentStrength, 0);
    const player2Strength = game.players.player2.monsterZone.reduce((sum, monster) => sum + monster.currentStrength, 0);
